...
class String {
	...
	public function toUpperCase() : String {
		var b2 = new hl.types.Bytes(size+1);
		b2.blit(0, bytes, 0, size+1);
		@:privateAccess b2.utf8Upper(0, size);
		return __alloc__(b2,size,length);
	}

	public function toLowerCase() : String {
		var b2 = new hl.types.Bytes(size+1);
		b2.blit(0, bytes, 0, size+1);
		@:privateAccess b2.utf8Lower(0, size);
		return __alloc__(b2,size,length);
	}
	...
	public function charAt(index : Int) : String {
		if( index < 0 || index >= length )
			return "";
		var p = bytes.utf8Pos(0, index);
		var k = bytes.utf8Pos(p, 1);
		return subBytes(p, k - p);
	}
